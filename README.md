...
Please note, rCLAMPS expects input text files to be encoded in the standard Unix/Linux/OSX format (i.e., without linefeed characters at the ends of lines as introduced by many Windows/DOS-based text editing applications).  If your input files were created/edited using a Windows/DOS-based text editor, it is highly recommended that you first convert your text files to the standard format by preproceesing them via dos2unix (or a similar tool) prior to running rCLAMPS.
